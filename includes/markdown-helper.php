...
function render_markdown($text)
{
    ...
    $html = preg_replace_callback('#<table>(.*?)</table>#s', function ($matches) {
        ...
        if (false === strpos($tableHtml, 'class="md-table"')) {
...
function text_to_markdown($text)
{
    ...
    return preg_replace('/([*_`#\[\](){}])/', '\$1', $markdown);
}
...
function convert_markdown_tables($text, &$tableMap)
{
    ...
    while ($i < count($lines)) {
        ...
        $looksLikeHeader = false !== strpos($line, '|');
        ...
        if ($looksLikeHeader && $looksLikeSeparator) {
            ...
            while ($i < count($lines)) {
                $rowLine = $lines[$i];
                if ('' === trim($rowLine) || false === strpos($rowLine, '|')) {
                    break;
                }
