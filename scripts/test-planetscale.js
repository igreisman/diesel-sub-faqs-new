...
async function testPlanetScaleConnection() {
  ...
  try {
    ...
    if (tables.length === 0) {
      ...
      console.log(
        "   Run: mysql -h <host> -u <user> -p -D submarine_faqs < database/schema.sql",
      );
    } else {
    ...
    try {
      ...
      console.log(
        "⚠️  FAQs table not found - run migration after schema setup",
      );
    }
    ...
    try {
      const searchResult = await query(`
            ...
            `);
      console.log(
        `✅ Full-text search: ${searchResult.length} results for 'submarine'`,
      );
    } catch (e) {
      console.log(
        "⚠️  Full-text search not available yet - needs data migration",
      );
    }
    ...
    console.log("\nNext steps:");
    console.log(
      "1. If no tables found, run: mysql -h <host> -u <user> -p < database/schema.sql",
    );
...
if (!process.env.DATABASE_URL) {
  ...
  console.error(
    'export DATABASE_URL="mysql://user:pass@host/submarine_faqs?ssl={\"rejectUnauthorized\":true}"',
  );
  process.exit(1);
}
