...
function performSearch(query) {
  ...
  resultsContainer.innerHTML =
    '<div class="text-center"><div class="loading"></div> Searching...</div>';
  ...
  fetch(`api/search.php?q=${encodeURIComponent(query)}`)
    ...
    .catch((error) => {
      ...
      resultsContainer.innerHTML =
        '<div class="alert alert-danger">Search temporarily unavailable. Please try again later.</div>';
    });
}
...
function displaySearchResults(data, container) {
  ...
  if (data.success && data.results && data.results.length > 0) {
    let html =
      '<h5>Search Results:</h5><div class="search-results"><ul class="list-group">';
